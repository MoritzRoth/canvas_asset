...
void main() {
	float previousInfluence = texSample2D(g_Texture0, v_TexCoord).r;
	...
	float currentInfluence = u_drawAlpha * u_mouseDown.x * (
		       u_mouseDown.y  * smoothstep(penRadius, penSmoothRadius, length(uv - cursor))
		 + NOT(u_mouseDown.y) * smoothstep(penRadius, penSmoothRadius, sdSegment(uv, cursor, lastCursor))
	);

	influence = max(influence, currentInfluence);
	gl_FragColor = influence + 0.5;
}
